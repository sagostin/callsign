...
func (h *Handler) ListInboundRoutes(ctx iris.Context) {
	...
	if err := h.DB.Where("tenant_id = ? AND direction = ?", middleware.GetTenantID(ctx), "inbound").Find(&routes).Error; err != nil {
...
func (h *Handler) CreateInboundRoute(ctx iris.Context) {
	ctx.StatusCode(http.StatusCreated)
	ctx.JSON(iris.Map{"message": "Inbound route created"})
}
...
func (h *Handler) ListOutboundRoutes(ctx iris.Context) {
	...
	var routes []models.Dialplan
	if err := h.DB.Where("tenant_id = ? AND direction = ?", middleware.GetTenantID(ctx), "outbound").Find(&routes).Error; err != nil {
		ctx.JSON(iris.Map{"data": []interface{}{}})
		return
	}
...
func (h *Handler) CreateOutboundRoute(ctx iris.Context) {
	ctx.StatusCode(http.StatusCreated)
	ctx.JSON(iris.Map{"message": "Outbound route created"})
}
...
func (h *Handler) ListDialPlans(ctx iris.Context) {
	...
	var plans []models.Dialplan
	if err := h.DB.Where("tenant_id = ?", middleware.GetTenantID(ctx)).Find(&plans).Error; err != nil {
		ctx.StatusCode(http.StatusInternalServerError)
		ctx.JSON(iris.Map{"error": "Failed to fetch dial plans"})
		return
	}

	ctx.JSON(iris.Map{"data": plans})
}
...
func (h *Handler) CreateDialPlan(ctx iris.Context) {
	...
	ctx.StatusCode(http.StatusCreated)
	ctx.JSON(iris.Map{"data": plan, "message": "Dial plan created"})
}
...
func (h *Handler) GetDialPlan(ctx iris.Context) {
	...
	if err := h.DB.Where("id = ? AND tenant_id = ?", id, middleware.GetTenantID(ctx)).First(&plan).Error; err != nil {
		ctx.StatusCode(http.StatusNotFound)
...
func (h *Handler) UpdateDialPlan(ctx iris.Context) {
	...
	ctx.JSON(iris.Map{"data": plan, "message": "Dial plan updated"})
}
...
func (h *Handler) DeleteDialPlan(ctx iris.Context) {
	...
	if err := h.DB.Where("id = ? AND tenant_id = ?", id, middleware.GetTenantID(ctx)).Delete(&models.Dialplan{}).Error; err != nil {
		ctx.StatusCode(http.StatusInternalServerError)
		ctx.JSON(iris.Map{"error": "Failed to delete dial plan"})
		return
	}

	ctx.JSON(iris.Map{"message": "Dial plan deleted"})
}
